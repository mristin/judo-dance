...
"""Check that the distribution and judodance/__init__.py are in sync."""
import os
import pathlib
import subprocess
import sys
...
def main() -> int:
    ...
    if setup_py_map["version"] != judodance.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in judodance/__init__.py is: "
            f"{judodance.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != judodance.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in judodance/__init__.py is: "
            f"{judodance.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != judodance.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in judodance/__init__.py is: "
            f"{judodance.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != judodance.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in judodance/__init__.py is: "
            f"{judodance.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != judodance.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in judodance/__init__.py, "
                f"but found: {judodance.__status__}"
            )
            success = False
